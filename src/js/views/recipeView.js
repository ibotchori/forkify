...
//Import fractionall package
import { Fraction } from "fractional";

//Clear recipe from UI
export const clearRecipe = () => (elements.recipe.innerHTML = "");

//Format Count from for examle: 1.5 to 1 1/2
const formatCount = (count) => {
  if (count) {
    ...
    //When count is Integer (2)
    if (!dec) return count;
    //When count is Float (0.5)
    if (int === 0) {
      const fr = new Fraction(count); //{numerator: 1, denumeerator: 2}
      return `${fr.numerator}/${fr.denominator}`; // 1/2
    } else {
      //When count is Float (2.5)
